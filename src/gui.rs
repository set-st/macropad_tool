...
impl MacropadApp {
    ...
    fn apply_layout(&mut self) {
        ...
        for i in 0..3 {
            ...
            for row in 0..(self.ui_rows as usize) {
                for col in 0..(self.ui_cols as usize) {
                    if row < data.macropad_data.layers[i].buttons.len() && col < data.macropad_data.layers[i].buttons[row].len() {
                        new_layer.buttons[row][col] = data.macropad_data.layers[i].buttons[row][col].clone();
                    }
                }
            }
            for knob in 0..(self.ui_knobs as usize) {
                if knob < data.macropad_data.layers[i].knobs.len() {
                    new_layer.knobs[knob] = data.macropad_data.layers[i].knobs[knob].clone();
                }
            }
        ...
        data.status_msg = "Layout applied. Save to write to file.".to_string();
...
impl eframe::App for MacropadApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        if self.last_conn_check.elapsed() > Duration::from_secs(2) {
        ...
        egui::TopBottomPanel::bottom("status_bar").show(ctx, |ui| {
            let data = DATA.lock().unwrap();
            ui.horizontal(|ui| {
                ui.label(egui::RichText::new(&data.status_msg).color(data.status_color));
            });
        });

        egui::SidePanel::left("side_panel").width_range(200.0..=250.0).show(ctx, |ui| {
            let (rows, cols, knobs, orientation, pid) = {
                let data = DATA.lock().unwrap();
                (data.macropad_data.device.rows, data.macropad_data.device.cols, data.macropad_data.device.knobs, data.macropad_data.device.orientation, data.connected_pid.unwrap_or(0x8840))
            };
            ...
            if self.ui_rows != rows || self.ui_cols != cols || self.ui_knobs != knobs || self.ui_orientation != orientation {
            ...
            ui.horizontal(|ui| {
                ...
                egui::ComboBox::from_id_salt("led_mode_cb")
                    .selected_text(current_mode_text)
                    .show_ui(ui, |ui| {
                        for (m, t) in modes {
                            ui.selectable_value(&mut self.led_mode, m, t);
                        }
                    });
            });
            ...
            ui.horizontal(|ui| {
                ui.label("Layer:");
                egui::ComboBox::from_id_salt("led_layer_cb")
                    .selected_text(format!("Layer {}", self.led_layer))
                    .show_ui(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.label("Color:");
                egui::ComboBox::from_id_salt("led_color_cb")
                    .selected_text(format!("{:?}", self.led_color))
                    .show_ui(ui, |ui| {
            ...
            if pid == 0x8890 {
                ui.label(egui::RichText::new("Note: Color changes might not work on 0x8890").italics().size(10.0).color(egui::Color32::KHAKI));
            }

            if ui.button("Apply LED").clicked() {
                let mut d = DATA.lock().unwrap();
                self.sync_temp_to_data(&mut d);
                let _ = Mapping::save(&d.macropad_data, "mapping.ron");
                let mode = self.led_mode;
                let color = self.led_color;
                let layer = self.led_layer;
                thread::spawn(move || {
                    let options = Options {
                        command: Command::ShowGui,
                        devel_options: DevelOptions { vendor_id: VENDOR_ID, product_id: None, address: None, out_endpoint_address: None, in_endpoint_address: None, interface_number: None },
                    };
                    match open_keyboard(&options) {
                        Ok(mut kb) => {
                            if let Err(e) = kb.set_led(mode, layer, color) {
                                Self::set_status(&format!("❌ LED Error: {}", e), egui::Color32::RED);
                            } else {
                                Self::set_status("✅ LED mode updated and saved!", egui::Color32::GREEN);
                            }
                        }
            ...
            if ui.add_sized([ui.available_width(), 40.0], egui::Button::new("💾 Save Config")).clicked() {
                let mut d = DATA.lock().unwrap();
                self.sync_temp_to_data(&mut d);
                match Mapping::save(&d.macropad_data, "mapping.ron") {
                    Ok(_) => {
                        let path = Mapping::config_path();
                        d.status_msg = format!("✅ Config saved to: {}", path.display());
                        d.status_color = egui::Color32::GREEN;
                    }
                    Err(e) => { d.status_msg = format!("❌ Save error: {}", e); d.status_color = egui::Color32::RED; }
                }
            }
            ui.add_space(10.0);
            if ui.add_sized([ui.available_width(), 40.0], egui::Button::new("🚀 Program Device").fill(egui::Color32::from_rgb(0, 80, 0))).clicked() {
                ...
                d.status_msg = "🚀 Programming device...".to_string();
                ...
                thread::spawn(move || {
                    ...
                    match open_keyboard(&options) {
                        Ok(mut kb) => {
                            match kb.program(&config) {
                                Ok(_) => Self::set_status("✅ Device programmed successfully!", egui::Color32::GREEN),
                                Err(e) => Self::set_status(&format!("❌ Program error: {}", e), egui::Color32::RED),
                            }
                        }
                        Err(e) => Self::set_status(&format!("❌ USB error: {}", e), egui::Color32::RED),
                    }
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.horizontal(|ui| {
                let mut d = DATA.lock().unwrap();
                for i in 0..3 {
                    if ui.selectable_label(d.current_layer_idx == i, format!("Layer {}", i + 1)).clicked() {
                        self.sync_temp_to_data(&mut d);
                        d.current_layer_idx = i;
            ...
            egui::ScrollArea::vertical().show(ui, |ui| {
                ...
                egui::Grid::new("button_grid").spacing([10.0, 10.0]).show(ui, |ui| {
                    for row in 0..r {
                        for col in 0..c {
                            ...
                            if ui.add_sized([100.0, 40.0], egui::Button::new(btn_text).selected(is_selected)).clicked() {
                                self.sync_temp_to_data(&mut d);
                                d.selection = Selection::Button(row, col);
                                self.sync_data_to_temp(&d);
                            }
                        }
                        ui.end_row();
                ...
                if k > 0 {
                    ui.add_space(20.0);
                    ui.heading("Rotary Encoders");
                    for i in 0..k {
                        ui.horizontal(|ui| {
                            ui.label(format!("Knob {}:", i+1));
                            for (part, label) in [(KnobPart::CCW, "CCW"), (KnobPart::Press, "Press"), (KnobPart::CW, "CW")] {
                                let val = match part {
                                    KnobPart::CCW => &d.macropad_data.layers[layer_idx].knobs[i].ccw.mapping,
                                    KnobPart::Press => &d.macropad_data.layers[layer_idx].knobs[i].press.mapping,
                                    KnobPart::CW => &d.macropad_data.layers[layer_idx].knobs[i].cw.mapping,
                                };
                                let is_selected = d.selection == Selection::Knob(i, part);
                                let btn_text = if val.is_empty() { label } else { val };
                                if ui.add(egui::Button::new(btn_text).selected(is_selected)).clicked() {
                                    self.sync_temp_to_data(&mut d);
                                    d.selection = Selection::Knob(i, part);
                                    self.sync_data_to_temp(&d);
                                }
                            }
                        });
                ...
                if d.selection != Selection::None {
                    ...
                    ui.horizontal(|ui| {
                        ui.label("Delay (ms):");
                        if ui.text_edit_singleline(&mut self.temp_delay_val).changed() { self.sync_temp_to_data(&mut d); }
                        ui.add_space(20.0); ui.label("Mapping:");
                        if ui.text_edit_singleline(&mut self.temp_editor_val).changed() { self.sync_temp_to_data(&mut d); }
                    });
                    ...
                    ui.horizontal_wrapped(|ui| {
                        ...
                        for m in Modifier::iter() {
                            let key = format!("{}-", m.to_string().to_lowercase());
                            if ui.small_button(&key).clicked() { key_to_insert = Some(key); }
                        }
                    });
                    ui.horizontal_wrapped(|ui| {
                        ui.label("Media:");
                        for m in MediaCode::iter() {
                            let key = m.to_string().to_lowercase();
                            if ui.small_button(&key).clicked() { key_to_insert = Some(key); }
                        }
                    });
                    ui.horizontal_wrapped(|ui| {
                        ui.label("More:");
                        for key in ["click", "rclick", "mclick", "wheelup", "wheeldown", "space", "enter", "backspace", "tab", "esc", ","] {
                            if ui.small_button(key).clicked() { key_to_insert = Some(key.to_string()); }
                        }
                    });
                    ...
                    ui.label(egui::RichText::new("Click a button in the grid above to edit its configuration").italics());
                }
            });
        });
